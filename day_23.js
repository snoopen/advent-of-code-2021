...
const homes = ['11', '15', '12', '16', '13', '17', '14', '18'];
...
function nextSteps(board, map, moveCount) {
  let next = [];
  let occupied = [];
  for (let i in board) {
    let p = board[i][0];
...
function recursiveSolve(board, map, solution, solutions, moveCount, score) {
  ...
  let next = nextSteps(board, map, moveCount);
  // l(next)
  if (next.length === 0) {
    // l('BOO!');
    return solutions;
  ...
  for (let step of next) {
    // if (solution.length === 0 && !(step.from === '13' && step.to === '3')) continue;
    // if (solution.length === 1 && !(step.from === '12' && step.to === '13')) continue;
...
function part1() {
  ...
  let xyToKey = {};
  // let board = [];
  let board = Array.from({ length: 19 }, (v, k) => '.');
  let result = 0;

  
  readInputMap(board, map, xyToKey);
  extendMap(map, xyToKey);
  // console.log(map[10].next);
  // console.log(map[12].next);
  // console.table(map['15'].next);
  // l(board);
  ...
  // board = ['.','.','.','.','.','.','.','.','.','A1','.','.','B1','C1','D1','A2','B2','C2','D2'];
  // board = ['.','.','.','.','.','.','.','.','.','B1','.','.','C1','B2','D1','A1','D2','C2','A2'];
  // board = [
  //   '.','.','.','B2','.','.','.','.','.','.','.',
  //   'B1','C1','.','D1',
  //   'A1','D2','C2','A2'
  ...
  // let n = nextSteps(board, map, {});
  // l(n);
  // l(solutions);
  // let scores = solutions.map(e=>e.reduce((p,v)=>p+v.score,0));
  // l(scores);
  // l(Math.min(...scores));
  // l(`was it 12521?`)
  let m = solutions[0];
  ...
  console.log(m);
  console.log(m.reduce((p,v)=>p+v.score,0));
